...
mydb = mysql.connector.connect(host="localhost", user="root", password="roshna#2001", database="grocery_shop")
...
class groceryItems:
    ...
    def listingTheGroceryItems(self):
        category = str(input("enter your category(rice/pulses/bathroom_accesories"))
...
class order:
    ...
    def orderTheItems(self):
        ...
        if isWhetherToOrderTheItems == 'y':
            mycursor.execute(' SELECT cart_details.cart_id, grocery_items.grocery_name, cart_details.quantity, cart_details.total_cost, grocery_items.offers ,grocery_items.total_quantity_available ,grocery_items.grocery_id,cart_details.customer_id FROM cart_details INNER JOIN grocery_items ON cart_details.grocery_id = grocery_items.grocery_id WHERE customer_id like %s',(self.customer_id,))
            ...
            while True:
                ...
                if(cartItemOption==0):
                    print("Thanks for ordering visit again")
                    exit()
                if cartItemOption !=0:
                    cartId = cartItems[cartItemOption - 1][0]
                    total = int(cartItems[cartItemOption - 1][5]) - int(cartItems[cartItemOption - 1][2])
                    g_id = cartItems[cartItemOption -1][6]
                    c_id = cartItems[cartItemOption-1][7]
                    ...
                    if paymentMethod == 'online':
                        ...
                        mydb.commit()
                    mycursor.execute("update grocery_items set total_quantity_available=%s where grocery_id=%s",(total,g_id, ))
                    mydb.commit()
                    mycursor.execute("delete from cart_details where cart_id=%s and customer_id=%s",(cartId ,c_id, ))
                    mydb.commit()
...
if __name__ == '__main__':
    ...
    customerValidation = customerDetails(customerName,password)
